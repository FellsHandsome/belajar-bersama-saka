...
@onready var countdown_panel = $"../CanvasGroup/CountdownTimer/Panel" # Untuk hide panel timer visual
...
func _ready():
	...
	# Inisialisasi soal
	for i in range(8):
		var soal_node = canvas_group.get_node("Imlasoal%d" % (i + 1))
		var mb = soal_node.get_node("MainButton")
		imlasoals.append(soal_node)
...
func show_soal(idx: int):
	for i in range(imlasoals.size()):
		imlasoals[i].visible = (i == idx)
		if i == idx:
...
func selesai_kuis():
	...
	# Hide panel visual timer (bukan node Timer)
	if countdown_panel:
